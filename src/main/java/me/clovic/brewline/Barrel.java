...
import me.clovic.brewline.api.events.barrel.BarrelCreateEvent;
...
import me.clovic.brewline.utility.BoundingBox;
...
public class Barrel extends BarrelBody implements InventoryHolder {
    ...
    public static void onUpdate() {
        ...
        for (UUID worldUuid : barrels.keySet()) {
            ...
            if (checkCounters.getOrDefault(worldUuid, 0) == 0 && numBarrels > 0) {
                ...
                new BarrelCheck().runTaskTimer(BrewlinePlugin.getInstance(), 1, 1);
            }
        }
    }
    ...
    public boolean hasPermsOpen(Player player, PlayerInteractEvent event) {
        if (isLarge()) {
            if (!player.hasPermission("brewline.barrel.open.big")) {
                lang.sendEntry(player, "Error_NoBarrelAccess");
                return false;
            }
        } else {
            if (!player.hasPermission("brewline.barrel.open.small")) {
                lang.sendEntry(player, "Error_NoBarrelAccess");
                return false;
            }
        }
        ...
        BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(accessEvent);
    ...
    public boolean hasPermsDestroy(Player player, Block block, BarrelDestroyEvent.Reason reason) {
        ...
        BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(destroyEvent);
        ...
        if (barrel == null) {
            ...
            if (barrel.getBrokenBlock(true) == null) {
                ...
                if (BarrelAsset.isBarrelAsset(BarrelAsset.SIGN, spigot.getType())) {
                    if (!player.hasPermission("brewline.barrel.create.small")) {
                        lang.sendEntry(player, "Perms_NoBarrelCreate");
                        return false;
                    }
                } else {
                    if (!player.hasPermission("brewline.barrel.create.big")) {
                        lang.sendEntry(player, "Perms_NoBigBarrelCreate");
                        return false;
                    }
                }
                BarrelCreateEvent createEvent = new BarrelCreateEvent(barrel, player);
                BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(createEvent);
                if (!createEvent.isCancelled()) {
                    barrels.computeIfAbsent(sign.getWorld().getUID(), ignored -> new ArrayList<>()).add(0, barrel);
                    return true;
                }
            }
        } else {
    ...
    private boolean computeSmall() {
        Preconditions.checkState(BrewlinePlugin.getScheduler().isRegionThread(spigot.getLocation()));
    ...
    public static CompletableFuture<Boolean> computeSmall(Location spigotPosition) {
        ...
        BrewlinePlugin.getScheduler().runTask(spigotPosition,
            ...
            barrels.keySet()
                .forEach(worldUuid -> {
                    ...
                    while (counter < worldBarrels.size()) {
                        ...
                        BrewlinePlugin.getScheduler().runTask(barrel.getSpigot().getLocation(), () -> {
