...
import me.clovic.brewline.lore.Base91DecoderStream;
import me.clovic.brewline.lore.Base91EncoderStream;
import me.clovic.brewline.lore.BrewLore;
import me.clovic.brewline.lore.LoreLoadStream;
import me.clovic.brewline.lore.LoreSaveStream;
import me.clovic.brewline.lore.NBTLoadStream;
import me.clovic.brewline.lore.NBTSaveStream;
import me.clovic.brewline.lore.XORScrambleStream;
import me.clovic.brewline.lore.XORUnscrambleStream;
...
import me.clovic.brewline.recipe.PotionColor;
...
public class Brew implements Cloneable {
    ...
    public void setStatic(boolean immutable, ItemStack potion) {
        ...
        if (BrewlinePlugin.getMCVersion().isOrEarlier(MinecraftVersion.V1_9) && currentRecipe != null && canDistill()) {
    ...
    public ItemStack createItem(@Nullable BRecipe recipe, boolean event, @Nullable Player player) {
        ...
        BrewlinePlugin.getInstance().getBreweryStats().metricsForCreate(true);
        return potion;
    }
    ...
    public void save(ItemStack item) {
        ItemMeta meta;
        if (!item.hasItemMeta()) {
            meta = BrewlinePlugin.getInstance().getServer().getItemFactory().getItemMeta(item.getType());
        } else {
    ...
    public void convertPre1_11(ItemStack item) {
        ...
        potionMeta.setBasePotionData(new PotionData(PotionType.THICK)); // UNCRAFTABLE
        BrewLore lore = new BrewLore(this, potionMeta);
        lore.removeEffects();
