...
public class BSealer implements InventoryHolder {
    public static final NamespacedKey TAG_KEY = new NamespacedKey(BrewlinePlugin.getInstance(), "SealingTable");
    ...
    public void clickInv() {
        contents = null;
        if (task == null) {
            task = BrewlinePlugin.getScheduler().runTaskTimer(BrewlinePlugin.getInstance(), this::itemChecking, 1, 1);
        }
    }
        ...
        for (int i = 0; i < slotTime.length; i++) {
            if (slotTime[i] > 20) {
                ...
                if (brew != null && !brew.isStripped()) {
                    ...
                    if (playerValid && BrewlinePlugin.getMCVersion().isOrLater(MinecraftVersion.V1_9)) {
    ...
    public static boolean isBSealer(Block block) {
        if (BrewlinePlugin.getMCVersion().isOrLater(MinecraftVersion.V1_14) && block.getType() == config.getSealingTableBlock()) {
    ...
    public static void registerRecipe() {
        ...
        if (!config.isCraftSealingTable() && recipeExists()) {
            unregisterRecipe();
            return;
        } else if (!config.isCraftSealingTable() || recipeExists() || BrewlinePlugin.getMCVersion().isOrEarlier(MinecraftVersion.V1_13)) {
            return;
        }

        ItemStack sealingTableItem = new ItemStack(config.getSealingTableBlock());
        ItemMeta meta = BrewlinePlugin.getInstance().getServer().getItemFactory().getItemMeta(config.getSealingTableBlock());
        if (meta == null) return;
        ...
        ShapedRecipe recipe = new ShapedRecipe(new NamespacedKey(BrewlinePlugin.getInstance(), "SealingTable"), sealingTableItem);
        recipe.shape("bb ",
            "ww ",
            "ww ");
