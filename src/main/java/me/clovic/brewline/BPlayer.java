...
import me.clovic.brewline.api.events.PlayerEffectEvent;
import me.clovic.brewline.api.events.PlayerPukeEvent;
import me.clovic.brewline.api.events.PlayerPushEvent;
import me.clovic.brewline.api.events.brew.BrewDrinkEvent;
...
import me.clovic.brewline.lore.BrewLore;
import me.clovic.brewline.recipe.BEffect;
...
import me.clovic.brewline.utility.PermissionUtil;
...
public class BPlayer {
    ...
    public static BPlayer getByName(String playerName) {
        for (Map.Entry<String, BPlayer> entry : players.entrySet()) {
            OfflinePlayer p = BrewlinePlugin.getInstance().getServer().getOfflinePlayer(UUID.fromString(entry.getKey()));
    ...
    public static boolean drink(Brew brew, Player player, @Nullable ItemMeta meta, @Nullable PlayerItemConsumeEvent event) {
        ...
        if (meta != null) {
            BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(drinkEvent);
        ...
        BrewlinePlugin.getInstance().getBreweryStats().forDrink(brew);
    ...
    public void showDrunkeness(Player player) {
        try {
            ...
            if (sendDrunkenessMessage(player)) {
                BrewlinePlugin.getScheduler().runTaskLater(() -> sendDrunkenessMessage(player), 40);
                BrewlinePlugin.getScheduler().runTaskLater(() -> sendDrunkenessMessage(player), 80);
            }
        } catch (Exception e) {
    ...
    public boolean sendDrunkenessMessage(Player player) {
        ...
        if (hangover && VERSION.isOrLater(MinecraftVersion.V1_11)) {
            BrewlinePlugin.getScheduler().runTaskLater(() -> player.sendTitle("", text, 30, 100, 90), 160);
            return false;
        }
        try {
    ...
    public void drinkCap(Player player) {
        ...
        drunkenness = 100;
        if (config.isEnableKickOnOverdrink() && !player.hasPermission("brewline.bypass.overdrink")) {
            BrewlinePlugin.getScheduler().runTaskLater(() -> passOut(player), 1);
        } else {
                    ...
                    if (event.getFrom().getX() != event.getTo().getX() || event.getFrom().getZ() != event.getTo().getZ()) {
                        ...
                        if (((Entity) player).isOnGround()) {
                            ...
                            if (time == 0) {
                                ...
                                BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(pushEvent);
    ...
    public void join(final Player player) {
        ...
        if (drunkenness < 10) {
            if (offlineDrunk > 60) {
                if (config.isEnableHome() && !player.hasPermission("brewline.bypass.teleport")) {
                    goHome(player);
                }
            }
            if (offlineDrunk > 20) {
            ...
            if (config.isEnableWake() && !player.hasPermission("brewline.bypass.teleport")) {
    ...
    public static void addPuke(Player player, int count) {
        ...
        BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(event);
        ...
        if (pTasks.isEmpty()) {
            task = BrewlinePlugin.getScheduler().runTaskTimer(player, BPlayer::pukeTask, 1L, 1L);
        }
    ...
    public static void puke(Player player) {
        ...
        item.setMetadata("brewery_puke", new FixedMetadataValue(BrewlinePlugin.getInstance(), true));
    ...
    public void drunkEffects(Player player) {
        ...
        for (PotionEffect effect : l) {
            BrewlinePlugin.getScheduler().runTask(player, () -> effect.apply(player)); // Fix can't add effect to entities Async
        }
    }
    ...
    public int getQuality() {
        if (drunkenness == 0) {
            // PAPI Placeholder %brewline_quality% may be used on players that aren't drunk!
