...
import me.clovic.brewline.configuration.annotation.DefaultCommentSpace;
import me.clovic.brewline.configuration.annotation.Footer;
...
import me.clovic.brewline.configuration.sector.WordsSector;
...
import me.clovic.brewline.configuration.sector.capsule.ConfigDistortWord;
...
@Header({ "!!! IMPORTANT: Brewline configuration files do NOT support external comments! If you add any comments, they will be overwritten !!!",
    "Keep a backup before editing recipes, storage, or command aliases." })
...
public class Config extends AbstractOkaeriConfigFile {
    ...
    public void onFirstCreation() {
        ...
        Logging.log("&9If this is your first time using Brewline, change config.yml#language to your language and run &6/brewline refresh");
    }
    ...
    private ReleaseChecker.ReleaseCheckerType resolveUpdatesFrom = ReleaseChecker.ReleaseCheckerType.NONE;

    @LocalizedComment("config.autosave")
    private int autosave = 10;

    @LocalizedComment("config.pluginPrefix")
    private String pluginPrefix = "&#36d7d0[&6Brewline&#36d7d0]&f ";
    ...
    private ConfiguredDataManager storage = ConfiguredDataManager.builder()
        ...
        .database("brewline-data")
        .tablePrefix("brewline_")
        .address("localhost")
        .username("root")
        .password("password")
        .build();
