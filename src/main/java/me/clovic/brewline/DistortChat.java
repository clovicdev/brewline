...
import me.clovic.brewline.api.events.PlayerChatDistortEvent;
...
public class DistortChat {
                    ...
                    if (command.length() + 1 < chat.length()) {
                        if (Character.isSpaceChar(chat.charAt(command.length()))) {
                            if (chat.toLowerCase().startsWith(command.toLowerCase())) {
                                ...
                                BrewlinePlugin.getInstance().getServer().getPluginManager().callEvent(call);
                                if (call.isCancelled()) {
                                    return;
                                }
    ...
    public static void playerChat(AsyncPlayerChatEvent event) {
        if (event.getPlayer().hasPermission("brewline.bypass.chatdistort")) {
            return;
        }
