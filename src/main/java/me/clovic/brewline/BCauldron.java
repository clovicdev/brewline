...
import me.clovic.brewline.api.events.IngedientAddEvent;
...
public class BCauldron {
    ...
    public boolean fill(Player player, Block block) {
        if (!player.hasPermission("brewline.cauldron.fill")) {
    ...
    public static void printTime(Player player, Block block) {
        if (!player.hasPermission("brewline.cauldron.time")) {
    ...
    public static void processCookEffects() {
        ...
        for (BCauldron cauldron : bcauldrons.values()) {
            if (particleRandom.nextFloat() < chance) {
                BrewlinePlugin.getScheduler().runTask(cauldron.block.getLocation(), cauldron::cookEffect);
            }
        }
    }
        ...
        if (MaterialUtil.isCauldronHeatSource(down)) {
            ...
            if (VERSION.isOrLater(MinecraftVersion.V1_9)) {
                if (event.getHand() == EquipmentSlot.HAND) {
                    ...
                    plInteracted.add(id);
                    BrewlinePlugin.getScheduler().runTask(() -> plInteracted.remove(id));
            ...
            if (!player.hasPermission("brewline.cauldron.insert")) {
    ...
    public static void reload() {
        ...
        var scheduler = BrewlinePlugin.getScheduler();
        for (BCauldron cauldron : bcauldrons.values()) {
    ...
    public static void unloadWorlds() {
        List<World> worlds = BrewlinePlugin.getInstance().getServer().getWorlds();
    ...
    public static void giveItem(final Player player, final ItemStack item) {
        BrewlinePlugin.getScheduler().runTaskLater(() -> player.getInventory().addItem(item), 1L);
    }
